...
public class AirportRepositoryJdbcImpl implements AirportRepository {
    ...
    public Airport getByIATAcode (String IATAcode) {
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource()
                .addValue("iata_code", IATAcode);
        try {
            return namedParameterJdbcTemplate.queryForObject("SELECT * FROM airports WHERE iata_code=:iata_code ORDER BY iata_code ASC",
    ...
    public Airport getByName (String name) {
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource()
                .addValue("name", name);
        try {
            return namedParameterJdbcTemplate.queryForObject("SELECT * FROM airports WHERE name=:name ORDER BY name ASC",
                    mapSqlParameterSource, this::mapRow);
        } catch (EmptyResultDataAccessException e) {
            return null;
        }
    ...
    public List<Airport> getByCountry (String country) {
        ...
        return namedParameterJdbcTemplate.query("SELECT * FROM airports WHERE country=:country ORDER BY country ASC",
                mapSqlParameterSource, this::mapRow);
    }

    @Override
    public List<Airport> getAll () {
        return namedParameterJdbcTemplate.query("SELECT * FROM airports ORDER BY iata_code ASC", this::mapRow);
    }
}
